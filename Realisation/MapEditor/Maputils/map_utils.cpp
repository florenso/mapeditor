...
RectInfo[][] RectInfo_from_map_using_tiles(Map map, int amountx, int amounty) {
    RectInfo[amounty][amountx] result;
    ...
    for (int y = 0; y < amounty; y++) {
        for (int x = 0; x < amountx; x++) {
            ...
            result[y][x] = RectInfo(current_box, new_type);
        }
    }
}

RectInfo[][] RectInfo_from_map_using_quadtree(Map map, Box size) {
